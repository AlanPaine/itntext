...
from itntext.grammars.text_normalization.zh.utils import (
    UNIT_1e01,
    UNIT_1e02,
    UNIT_1e03,
    UNIT_1e04,
...
class Cardinal(GraphFst):
    ...
    def __init__(self, deterministic: bool = True, lm: bool = False):
        ...
        graph_phone_digit = pynini.string_map(
            [
                ("0", "零"),
                ("1", "幺"),
                ("2", "二"),
        ...
        graph_long_numstring = pynutil.insert('integer: "') + pynini.closure(graph_phone_digit, 7) + pynutil.insert('"')
        graph_cardinal = self.add_tokens(
            graph_long_numstring | (pynutil.insert('integer: "') + self.graph_cardinal + pynutil.insert('"'))
        )
        self.fst = graph_cardinal.optimize()
