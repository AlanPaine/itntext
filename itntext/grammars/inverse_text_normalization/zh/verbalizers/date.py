...
class DateFst(GraphFst):
    ...
    def __init__(self):
        ...
        strip_leading_zero = pynini.string_map(
            [(f"0{i}", str(i)) for i in range(1, 10)] + [(str(i), str(i)) for i in range(1, 32)]
        )
        month = (
            pynutil.delete("month:")
            + delete_space
            + pynutil.delete('"')
            + (pynini.closure(DAMO_NOT_QUOTE, 1) @ strip_leading_zero)
            + pynutil.delete('"')
        )
        day = (
            pynutil.delete("day:")
        ...
        optional_suffix = suffix | pynutil.insert("日")
        ...
        graph_md = month + pynini.cross(" ", "月") + day + optional_suffix
